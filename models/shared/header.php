...
class Header extends Model {
    ...
    public function main_menu() {
        ...
        $main_menu  = Settings::get_setting( 'main_menu' );
        $main_links = $main_menu['main_links'] ?? [];
        ...
        foreach ( $main_links as $main_link_acf ) {

            if ( empty( $main_link_acf['main_link'] ) ) {
                continue;
            }

            $current_parent_class_added = 0;
            ...
            $main_link_array['link'] = $main_link_acf['main_link'];

            if ( $main_link_acf['main_link']['url'] === $current_url ) {

                $main_link_array['class'] .= ' current-menu-item';
            }

            /**
             * Go through submenu links.
             */
            if ( ! empty( $main_link_acf['use_submenu'] ) && ! empty( $main_link_acf['submenu_links'] ) ) {

                $main_link_array['use_submenu'] = true;

                $main_link_array['class'] .= ' has-dropdown menu-item-has-children';

                foreach ( $main_link_acf['submenu_links'] as $submenu_link_acf ) {
                    ...
                    if ( empty( $submenu_link_acf['submenu_link']['url'] ) ) {
                        continue;
                    }

                    $submenu_link['link'] = $submenu_link_acf['submenu_link'];

                    if ( $submenu_link_acf['submenu_link']['url'] === $current_url ) {

                        $submenu_link['class'] .= ' is-current';

                        if ( ! $current_parent_class_added ) {
                            $main_link_array['class'] .= ' current-menu-parent';
                        }

                        $current_parent_class_added = 1;
                    }

                    $main_link_array['submenu_links'][] = $submenu_link;
                }
            }
            else {
                $main_link_array['class'] .= ' navbar-item__single';
            }

            if ( $main_link_array['use_submenu'] && $main_link_acf['main_link']['url'] === '#' ) {
                $main_link_array['class'] .= ' navbar-item--trigger-only';
            }

            $formatted_links[] = $main_link_array;
        }
